<h1 align="center">Hi, I'm Sergio 👋</h1>

<p align="center">
  <b>iOS Developer · Swift &amp; SwiftUI</b><br>
  Graduate of the Swift Developer Program at Apple Coding Academy
</p>

<p align="center">
  <a href="https://sergiogc.dev"><img src="https://img.shields.io/badge/Portfolio-007AFF?style=for-the-badge&logo=safari&logoColor=white" alt="Portfolio"></a>
  &nbsp;
  <a href="https://sergiogc.dev/cv"><img src="https://img.shields.io/badge/CV-111110?style=for-the-badge&logo=readthedocs&logoColor=white" alt="CV"></a>
  &nbsp;
  <a href="https://www.linkedin.com/in/sergiogarciacalero/"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"></a>
</p>

---

### 🚀 About me

After 8 years in tech support, I made the jump into something I'd always been drawn to: building software. I recently completed the **Swift Developer Program** at Apple Coding Academy, and now I build **100% native iOS apps** with Swift and SwiftUI — no templates, no shortcuts.

I believe the best way to improve is by **building in public**, so everything here reflects both my progress and my curiosity. Expect experiments, real projects, and plenty of learning along the way. 🌱

---

### 🛠️ Tech I work with
<p>
  <div style="text-align: center;">
    <a href="https://skillicons.dev">
      <img src="https://skillicons.dev/icons?i=swift,xcode,git,sql-server,sqlite,lua" />
    </a>
  </div>
</p>
---

### 📱 Featured projects

<table>
  <tr>
    <td width="33%" valign="top">
      <h4>📚 TankōDex</h4>
      <p>My SDP final project. Track your manga collection through an external API, with advanced search and a personal library.</p>
      <a href="https://github.com/SergioGCDev/PRACTICA-FINAL-SDP-26">→ View repo</a>
    </td>
    <td width="33%" valign="top">
      <h4>💸 FinMate</h4>
      <p>A personal finance companion: track daily expenses, subscriptions and contracts, keeping your money always visible.</p>
      <a href="https://github.com/SergioGCDev/FinMate">→ View repo</a>
    </td>
    <td width="33%" valign="top">
      <h4>💊 SuppleMate</h4>
      <p>Daily supplement &amp; medication tracking with smart reminders and a personal intake history.</p>
      <a href="https://github.com/SergioGCDev/SuppleMate">→ View repo</a>
    </td>
  </tr>
</table>

---

<p align="center">
  <i>Currently building, learning, and shipping — one commit at a time.</i> 🚀
</p>
